  ...
  return (
    <main className="min-h-screen bg-gray-50 px-6 py-16">
      <div className="mx-auto max-w-5xl">
        ...
        <h1 className="mt-6 text-3xl font-bold">Exams</h1>
        <p className="mt-2 text-gray-600">
          Choose an exam entry to explore its published mock tests.
        </p>
        ...
        {exams.length === 0 ? (
          ...
          <div className="mt-8 space-y-6">
            {exams.map((exam) => {
              const entries = groups.filter(
                (group) => group.exam_id === exam.id
              );

              return (
                <section key={exam.id} className="rounded-2xl border bg-white p-6">
