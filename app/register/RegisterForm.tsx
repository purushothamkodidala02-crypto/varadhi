  ...
  return (
    <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-600">
        It takes less than a minute. You can start with any published free mock test.
      </p>
      <form onSubmit={handleRegister} className="mt-7 space-y-5">
        ...
        {message && (
          <p
            aria-live="polite"
            className={`rounded-xl border px-4 py-3 text-sm font-medium ${success ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-700"}`}
          >
            {message}
          </p>
        )}
