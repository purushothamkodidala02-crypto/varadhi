...
export default async function MockTestsPage({ searchParams }: MockTestsPageProps) {
  ...
  const [categoriesResult, examsResult, specializationsResult, papersResult, subjectsResult, testsResult, assignmentsResult] = await Promise.all([
    ...
    supabase.from("papers").select("id, exam_group_id, specialization_id, name, display_order").eq("is_active", true).order("display_order"),
  ...
  const tests = (testsResult.data ?? []).map((test) => {
    ...
    return { ...test, paper, paperDisplay, exam, category, specialization, subject, stats: statsByTest.get(test.id) ?? { questions: 0, marks: 0 } };
  }).filter((test) =>
