...
export default async function AdminQuestionsPage() {
  ...
  const [questionsResult, subjectsResult, groupsResult, examsResult] = await Promise.all([
    supabase.from("questions").select("id, subject_id, question_text, question_type, option_a, option_b, option_c, option_d, correct_answer, explanation, difficulty, image_url, source_reference, is_active, created_at, updated_at").order("created_at", { ascending: false }),
  ...
  const questionRows: QuestionBankRow[] = questions.map((question) => {
    ...
    return {
      ...
      subjectKey: question.subject_id ?? "general",
    };
  });
  ...
  return (
    <div>
      <header className="flex flex-wrap items-end justify-between gap-5">
        <div><p className="text-xs font-bold uppercase tracking-[0.15em] text-teal-700">Content library</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Question bank</h1><p className="mt-3 max-w-2xl leading-7 text-slate-600">Create questions, find them quickly, and assign them to any draft mock test.</p></div>
      ...
      <section className="mt-7 grid gap-4 sm:grid-cols-3">{[["Total questions", questions.length, "In your library"], ["Active and ready", activeQuestions, "Available to assign"], ["Subjects", subjects.length, "Places to organise content"]].map(([label, value, description]) => <div key={label as string} className="rounded-2xl border bg-white p-5 shadow-sm"><p className="text-sm font-bold text-slate-500">{label}</p><p className="mt-2 text-3xl font-black text-slate-950">{value}</p><p className="mt-2 text-xs font-medium text-slate-500">{description}</p></div>)}</section>
