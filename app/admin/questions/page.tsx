...
export default async function AdminQuestionsPage() {
  ...
  const groupOptions = groups
    .map((group) => ({ id: group.id, order: group.display_order, label: `${examMap.get(group.exam_id)?.name ?? "Unknown exam"} — ${group.name}` }))
  ...
  const questionRows: QuestionBankRow[] = questions.map((question) => {
    ...
    return {
      ...
      suitableGroupLabels: suitableGroupsByQuestion.get(question.id) ?? [],
    };
  });
  ...
  return (
    <div>
      ...
      <CreateQuestionForm subjects={subjectOptions} groups={groupOptions} />
