...
import { createExamState, deleteExamState, toggleExamState, type StateActionResult } from "@/app/admin/states/actions";
...
export function StateManager({ states }: { states: ManagedState[] }) {
  const [result, action, pending] = useActionState(createExamState, initialState);
  return <details className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="border-t bg-slate-50/70 p-5 sm:p-7">
      <div className="grid gap-3 md:grid-cols-3">{states.map((state) => <article key={state.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-teal-50 text-teal-800"><StateSymbol slug={state.slug} className="h-5 w-5" /></span><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${state.isActive ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{state.isActive ? "Visible" : "Hidden"}</span></div><div className="mt-4 flex items-baseline gap-2"><h3 className="font-display text-lg">{state.name}</h3><span className="text-xs font-black text-teal-700">{state.code}</span></div><p className="mt-1 text-xs text-slate-500">/{state.slug} · {state.categoryCount} boards</p><div className="mt-4 flex gap-2 border-t pt-4"><form action={toggleExamState}><input type="hidden" name="id" value={state.id} /><input type="hidden" name="is_active" value={String(state.isActive)} /><button className="rounded-lg border px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">{state.isActive ? "Hide" : "Show"}</button></form>{state.categoryCount === 0 && <form action={deleteExamState}><input type="hidden" name="id" value={state.id} /><button className="rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-50">Delete</button></form>}</div></article>)}</div>
      <form action={action} className="mt-6 grid gap-4 rounded-2xl border border-teal-100 bg-white p-5 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex items-end"><button disabled={pending} className="w-full rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50">{pending ? "Adding…" : "Add catalogue"}</button></div>
