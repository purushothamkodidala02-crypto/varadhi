  ...
  return (
    <section className="mt-8 rounded-2xl border bg-white p-6 shadow-sm">
      <form action={formAction} className="space-y-5">
        ...
        <section className="rounded-xl border bg-slate-50 p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-bold">Current Papers</h3><p className="mt-1 text-sm text-slate-600">Manage an existing Paper’s details from the Papers page.</p></div><a href="/admin/papers" className="text-sm font-bold text-teal-700 hover:underline">Manage Papers</a></div>{papers.length === 0 ? <p className="mt-4 text-sm text-slate-600">No Papers added yet.</p> : <div className="mt-4 grid gap-3 md:grid-cols-2">{papers.map((paper) => <div key={paper.id} className="rounded-lg border bg-white px-4 py-3"><p className="font-semibold">{paper.name}</p><p className="mt-1 text-xs text-slate-500">{paper.question_count ? `${paper.question_count} questions` : "Question count not set"}{paper.duration_minutes ? ` · ${paper.duration_minutes} min` : ""}{!paper.is_active ? " · Inactive" : ""}</p></div>)}</div>}</section>
