  ...
  return (
    <section className="rounded-xl border border-teal-100 bg-teal-50/50 p-5">
      ...
      {papers.length === 0 ? <p className="mt-4 rounded-lg border border-dashed border-teal-200 bg-white/70 p-3 text-sm text-slate-600">No new Papers selected. Use Add Paper when this Exam needs another Paper.</p> : <div className="mt-4 space-y-3">{papers.map((paper, index) => {
        ...
        return <div key={paper.key} className="rounded-xl border bg-white p-4">
          <label className="block text-sm font-semibold">
            Paper {index + 1} name
            <input required={initialRows > 0} value={paper.name} onChange={(event) => updatePaper(paper.key, "name", event.target.value)} placeholder="For example: Paper I - General Studies" className="mt-1.5 w-full rounded-lg border px-3 py-2.5 font-normal" />
          </label>
          <div
            className="mt-4 grid gap-3"
            style={{ gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 11rem), 1fr))" }}
          >
            <label className="block text-sm font-semibold">Minutes <span className="font-normal text-slate-500">optional</span><input type="number" min="1" value={paper.duration_minutes} onChange={(event) => updatePaper(paper.key, "duration_minutes", event.target.value)} placeholder="For example: 150" className="mt-1.5 w-full rounded-lg border px-3 py-2.5 font-normal" /></label>
            <label className="block text-sm font-semibold">Questions <span className="font-normal text-slate-500">optional</span><input type="number" min="1" value={paper.question_count} onChange={(event) => updatePaper(paper.key, "question_count", event.target.value)} placeholder="For example: 150" className="mt-1.5 w-full rounded-lg border px-3 py-2.5 font-normal" /></label>
            <label className="block text-sm font-semibold">Correct marks<input type="number" min="0.01" step="0.01" required value={paper.default_correct_marks} onChange={(event) => updatePaper(paper.key, "default_correct_marks", event.target.value)} placeholder="For example: 1" className="mt-1.5 w-full rounded-lg border px-3 py-2.5 font-normal" /></label>
            <label className="block text-sm font-semibold">Negative marks<input type="number" min="0" step="0.01" required value={paper.default_negative_marks} onChange={(event) => updatePaper(paper.key, "default_negative_marks", event.target.value)} placeholder="For example: 0.25" className="mt-1.5 w-full rounded-lg border px-3 py-2.5 font-normal" /></label>
          </div>
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            {paper.question_count && Number.isFinite(totalMarks) ? <p className="text-xs font-semibold text-teal-800">Total Paper marks: {totalMarks}</p> : <span />}
            <button type="button" onClick={() => setPapers((current) => current.filter((item) => item.key !== paper.key))} className="text-sm font-semibold text-red-600 hover:underline">Remove Paper</button>
          </div>
        </div>;
      })}</div>}
