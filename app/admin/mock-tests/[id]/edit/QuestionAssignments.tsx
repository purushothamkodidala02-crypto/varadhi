...
export function QuestionAssignments({ mockTestId, isDraft, availableQuestions, assignedQuestions }: QuestionAssignmentsProps) {
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-3xl border bg-white shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 bg-slate-50 px-6 py-5 sm:px-7"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-teal-700">Test builder</p><h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Questions in this mock test</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Search active questions for this subject, preview the selection, then add it with the correct marks.</p></div><span className="rounded-full bg-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700">{assignedQuestions.length} assigned</span></div>

      {isDraft ? <form action={formAction} className="border-b border-slate-100 p-6 sm:p-7"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-black text-slate-950">Add a question</p><p className="mt-1 text-sm text-slate-600">Only unassigned active questions from this subject are shown.</p></div><Link href="/admin/questions#add-question" className="text-sm font-bold text-teal-700 hover:text-teal-800">+ Create a new question</Link></div>
