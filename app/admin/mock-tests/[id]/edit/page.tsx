...
export default async function EditMockTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main>
      ...
      <MockTestCsvImport mockTestId={test.id} isDraft={test.status === "draft"} paperName={testPaper?.name ?? "this Paper"} subjectName={test.subject_id ? subjectById.get(test.subject_id)?.name ?? null : null} />
      <QuestionAssignments mockTestId={test.id} isDraft={test.status === "draft"} availableQuestions={availableQuestions} assignedQuestions={assignments} defaultMarks={testPaper?.default_correct_marks ?? 1} defaultNegativeMarks={testPaper?.default_negative_marks ?? 0} />
    </main>
  );
}
