...
export default async function EditMockTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><Link href="/admin/mock-tests" className="text-sm font-semibold text-teal-700 hover:underline">&lt;- Back to Mock Tests</Link><h1 className="mt-5 text-3xl font-black">Build Mock Test</h1><p className="mt-2 text-slate-600">Upload a CSV to build this draft quickly, or choose individual Questions from the Question Bank.</p><EditMockTestForm mockTest={test} papers={papers.map((paper) => { const group = groups.get(paper.exam_group_id); return { id: paper.id, label: `${categories.get(group?.exam_id ?? "")?.name ?? "Unknown category"} -> ${group?.name ?? "Unknown Exam"}${paper.specialization_id ? ` -> ${specializations.get(paper.specialization_id) ?? "Unknown Specialisation"}` : ""} -> ${paper.name}`, duration: paper.duration_minutes }; })} subjects={subjects.map((subject) => ({ id: subject.id, paperId: subject.paper_id, name: subject.name }))} /><MockTestCsvImport mockTestId={test.id} isDraft={test.status === "draft"} paperName={testPaper?.name ?? "this Paper"} subjectName={test.subject_id ? subjectById.get(test.subject_id)?.name ?? null : null} /><QuestionAssignments mockTestId={test.id} isDraft={test.status === "draft"} availableQuestions={availableQuestions} assignedQuestions={assignments} defaultMarks={testPaper?.default_correct_marks ?? 1} defaultNegativeMarks={testPaper?.default_negative_marks ?? 0} /></main>;
}
