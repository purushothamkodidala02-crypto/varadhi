...
export default async function EditPaperPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ fromExam?: string }> }) {
  ...
  const [paperResult, groupsResult, categoriesResult, specializationsResult] = await Promise.all([
    supabase.from("papers").select("id, exam_group_id, specialization_id, name, slug, description, duration_minutes, question_count, default_correct_marks, default_negative_marks, is_active, display_order").eq("id", id).maybeSingle(),
  ...
  return <main><Link href={backHref} className="text-sm font-semibold text-teal-700 hover:underline">{backLabel}</Link><h1 className="mt-5 text-3xl font-black">Edit Paper</h1><PaperForm paper={paper} exams={groups.map((group) => ({ id: group.id, label: `${categories.get(group.exam_id) ?? "Unknown category"} → ${group.name}` }))} specializations={(specializationsResult.data ?? []).map((item) => ({ id: item.id, examId: item.exam_group_id, name: item.name }))} /></main>;
}
