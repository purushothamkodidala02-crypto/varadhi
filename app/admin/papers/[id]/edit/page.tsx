...
export default async function EditPaperPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [paperResult, groupsResult, categoriesResult] = await Promise.all([supabase.from("papers").select("id, exam_group_id, name, slug, description, duration_minutes, question_count, default_correct_marks, default_negative_marks, is_active, display_order").eq("id", id).maybeSingle(), supabase.from("exam_groups").select("id, exam_id, name").order("display_order"), supabase.from("exams").select("id, name")]);
