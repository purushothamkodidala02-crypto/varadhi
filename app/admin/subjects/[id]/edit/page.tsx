...
export default async function EditSubjectPage({ params }: PageProps<"/admin/subjects/[id]/edit">) {
  ...
  const [subjectResult, papersResult, groupsResult, categoriesResult] = await Promise.all([supabase.from("subjects").select("id, paper_id, name, slug, description, is_active, display_order, created_at, updated_at").eq("id", id).maybeSingle(), supabase.from("papers").select("id, exam_group_id, name").order("display_order"), supabase.from("exam_groups").select("id, exam_id, name"), supabase.from("exams").select("id, name")]);
